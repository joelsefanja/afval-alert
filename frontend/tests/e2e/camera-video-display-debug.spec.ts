...
test.describe('Camera Video Display Debugging', () => {
  ...
  test('should show error when camera fails', async ({ page }) => {
  ...
  test('should handle camera constraints properly', async ({ page }) => {
    let capturedConstraints: any = null;
    
    // Capture the constraints passed to getUserMedia
    await page.addInitScript(() => {
      ...
      navigator.mediaDevices.getUserMedia = async function(constraints) {
        ...
        return originalGetUserMedia.call(this, { video: true, audio: false });
      };
    });
    ...
    capturedConstraints = await page.evaluate(() => (window as any).capturedConstraints);
    
    console.log('Captured camera constraints:', capturedConstraints);
    
    // Verify constraints are reasonable
    expect(capturedConstraints).toBeTruthy();
    expect(capturedConstraints.video).toBeTruthy();
    expect(capturedConstraints.audio).toBe(false);
    
    if (typeof capturedConstraints.video === 'object') {
      // Should have reasonable video constraints
      expect(capturedConstraints.video.facingMode).toBeTruthy();
    }
  });
});